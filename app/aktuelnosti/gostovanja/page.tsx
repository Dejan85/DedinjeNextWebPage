...
const SOURCES = Array.from(new Set(GOSTOVANJA.map((g) => g.source)));
...
export default function GostovanjaPage() {
  ...
  const featured = GOSTOVANJA[0];
  const rest = GOSTOVANJA.slice(1);

  const filtered = useMemo(() => {
    const fromDate = dateFrom ? new Date(dateFrom) : null;
    const toDate = dateTo ? new Date(dateTo + "T23:59:59") : null;

    const items = rest.filter((item) => {
      const matchSearch =
        !search ||
