...
- ✅ Dva dodatna bug-fixa istog dana (poslednji u nizu), prijavljena screenshotom vlasnika sajta — homepage "Наш тим" sekcija i dalje ćirilica na `/en`, Hipokratija widget nestaje pri promeni locale-a bez ručnog refresh-a:
  - `app/[locale]/page.tsx` (Team render, oko linije 885) — `member.name` (plain string) nikad nije dobio isti `locale === "en" ? transliterate(member.name) : member.name` tretman kao `testimonial.authorName` (2026-08-11); dodat sad.
  - `scripts/migrate-homepage-team-en-update.ts` (`npm run migrate:homepage-team-en-update`) — `teamSection.team[].role`/`.description` imali `en: ""` prazno od `migrate-homepage-team-update.ts` (2026-08-05); dopisan engleski prevod za sva 4 člana tima, pokrenuto uživo na produkciji (8 polja).
  - `components/shared/HipokratijaWidget/HipokratijaWidget.tsx` — root cause potvrđen dekompajliranjem vendor bundle-a: modul se izvršava jednokratno pri učitavanju (`document.querySelectorAll(".hipokratija-widget")` + fetch, bez `MutationObserver`-a), a Next-ov `next/script` dedup-uje po `src` pa se stari statički `<Script>` tag izvršio samo JEDNOM po browser sesiji — promena locale-a remountuje komponentu, ali vendor skripta se više ne pokreće. Rešenje: komponenta sama injektuje `<script type="module">` sa cache-bust `?_=Date.now()` query stringom u sopstvenom `useEffect`-u (svaki mount, uklj. promenu locale-a), cleanup uklanja tag pri unmount-u; stari `<Script>`/`next/script` import uklonjen iz `app/[locale]/page.tsx`.
  - Detaljno: vidi `PROJECT_STATUS.md` Dnevnik, red "2026-08-12 — Dva dodatna homepage bag-fixa..." — verifikovano `npx tsc --noEmit` čisto, `npm run lint` 28 problema (baseline nepromenjen, nula novih).
- ✅ Treći bug-fix istog dana — **svih 302 WordPress vesti nisu migrirane u i18n šemu** — veliki nalaz, drugačiji od prethodna dva po obimu i root cause-u. Faza 3d/3e su pokrile 99 dokumenata koji su postojali PRE WordPress migracije 2026-08-05; WordPress import toga dana nikad nije prošao kroz Fazu 3c/3d konverziju. Rezultat: svih 302 `news` dokumenata ima `title`/`excerpt`/`fullText`/`author` kao obične stringove umesto `{_type:"localeString"/"localeText", sr, en}` objekata — `localize()` vraća identičan SR tekst na oba locale-a (`/en` prikazuje ćirilicu). **Rešenje:** (1) `npm run migrate:i18n-schema -- --type=news` — ponovo pokrenut postojeći alat iz Faze 3c, sada sa `--type=news` flagom, konvertovao sva 4 polja (title/excerpt/fullText/author) na svih 302 dokumenta u `{_type,sr,en:""}` oblik (1208 polja ukupno, `--dry-run` potvrđen 302/302 pre upisa). (2) Batch prevod title+excerpt (604 polja — fullText i author NAMERNO ostavljeni bez prevoda za sada, veliki dodatni obim, van trenutnog zahteva — i dalje `{sr,en:""}` za budući prevod) kroz paralelnih 6 Agent poziva, svi sa formalnim institucionalno/novinarskim engleskim (pun naziv sa „ ", lična imena transliterisana, akademske titule prevedene, ALL-CAPS WordPress naslovi normalizovani na Title Case, odsečeni excerpt-i prevedeni kao odsečeni). Rezultati spojeni i validirani skriptom — 604/604 pokriveno, 0 nedostajućih/praznih. (3) `npm run i18n:apply-translations` — upisane EN vrednosti za svih 302 dokumenata + `enReviewed: false` (isti review gate kao ostatak sajta). `npx tsc --noEmit` čisto, `npm run lint` nepromenjen (kodo fix je infrastrukturniji — samo postojeći alati korišćeni, bez novih fajlova). **Watch-item:** `fullText` (pun tekst članka) i `author` ostaju bez prevoda, poznato i namerno — i dalje `{sr,en:""}` forma pa je `localize()` ispravno pada nazad na SR dok se ne prevede. Razlika od Faze 3d: ta faza je pokrila 99 dokumenata/2742 polja (postojeći), ova patch pokriva 302 vesti/1208 polja (iz WP migracije), odsečena na title+excerpt (604 polja za batch prevod). **Razlika u tvrdnji:** vesti su "migrirane" (sadržaj sa WP-a) ali nisu bila "lokalizovana" (i18n šema/prevod) — dva različita procesa.
  - Detaljno: vidi `PROJECT_STATUS.md` Dnevnik, red "2026-08-12 — Treći nalaz: svih 302 WordPress vesti bez i18n..." — ceo proces verifikovan na produkciji kroz upite (sva 302 `title.en`/`excerpt.en` popunjena) i dev server sa Sanity fetch kešom.
...
- 🟡 **Homepage (`app/page.tsx`) je delom demo-seed** iz `scripts/migrate-all.ts` — potvrđeno direktnim upitom na javni Sanity API (ne pretpostavka). Detalji u `popunjene-stranice-2026-07-21.md` → "Audit početne strane". Ukratko:
  ...
  - ✅ **Stats sekcija ažurirana realnim brojkama (2026-08-05)** — 19.000 procedura/god. (ranije 15.000 operacija), 200 lekara specijalista (nepromenjeno), 50 god. iskustva (ranije 65), 146.000 poseta/god. (ranije 50.000 "zadovoljnih pacijenata", label promenjen). Brojke dobijene od vlasnika sajta. Nova skripta `scripts/migrate-homepage-stats-update.ts` (`npm run migrate:homepage-stats-update`) patch-ovala `statsSection.stats` na produkciji, seed u `migrate-all.ts` usklađen.
    - **✅ i18n ispravka: stats sekcija label polja lokalizovana (2026-08-12)** — `stats[].label` polja su već bila ispravno tipizovana kao `localeString` (šema je bila u redu), ali `scripts/migrate-homepage-stats-update.ts` (patch skripta sa 2026-08-05) je pisala `en: ""` — prazan prevod, bez pratećeg `-en-update` pandana kao kod ostalih sekcija. Usled toga, 4 stat pilule na `/en` su prikazivale ćirilicu (SR fallback) umesto engleskog teksta. Nova ciljana patch skripta `scripts/migrate-homepage-stats-en-update.ts` (`npm run migrate:homepage-stats-en-update`) — `.set()` na `statsSection.stats[_key=="stat-X"].label.en` putanjama za sve 4 stata (stat-1 do stat-4), engleske vrednosti: "PROCEDURES PER YEAR", "SPECIALIST PHYSICIANS", "YEARS OF EXPERIENCE", "VISITS PER YEAR". Pokrenuta uživo, 4 polja ažurirana. Verifikovano direktnim upitom na produkcioni dataset — svi 4 labela sad na engleskom. `scripts/migrate-homepage-stats-update.ts` (originalna seed skripta) takođe ažurirana da upisuje prave `label.en` vrednosti za budući re-seed bez regresije. `npm run lint` (28, baseline nepromenjen).
